...
`initDB()` runs on app launch — creates tables if not exist, seeds data if words table is empty.

### Default Profile
On first launch, create a default adult profile: `{ name: 'Me', is_kid: 0, theme: 'midnight' }`

## Files to Create/Modify
- `apps/more-words/src/db/database.ts` — database module
